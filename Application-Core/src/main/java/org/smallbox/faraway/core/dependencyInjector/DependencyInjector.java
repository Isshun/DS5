...
public class DependencyInjector implements GameObserver {
...
//        new Reflections("org.smallbox.faraway").getTypesAnnotatedWith(Component.class)
//                .forEach(cls -> {
//                    Log.info("Find component: " + cls.getSimpleName());
//
//                    _handlers.entrySet().stream()
//                            .filter(entry -> entry.getKey().getAnnotation(ComponentHandlerAnnotation.class).type().isAssignableFrom(cls))
//                            .forEach(entry -> entry.getValue().invoke(cls));
//                });
