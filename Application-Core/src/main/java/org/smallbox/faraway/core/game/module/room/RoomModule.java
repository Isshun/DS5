...
public class RoomModule extends GameModule implements GameObserver {
    ...
    public void refreshRooms(int floor) {
        Log.info("org.smallbox.faraway.core.game.module.room.RoomModule: refresh floor " + floor);
...
//                        Log.info("Create new room for parcel " + x + "x" + y);
        ...
        Log.info("org.smallbox.faraway.core.game.module.room.RoomModule: refresh done " + (System.currentTimeMillis() - time));
    }
...
//        room.setOxygen(oxygen / nbParcel);
    ...
    private void checkRoof(RoomModel room) {
        printInfo("[org.smallbox.faraway.core.game.module.room.RoomModule] Check roof: " + room.getName());
        for (ParcelModel parcel: room.getParcels()) {
            ...
            if (!isRoofSupported) {
                printInfo("[org.smallbox.faraway.core.game.module.room.RoomModule] roof collapse");
                room.setExterior(true);
                return;
            }
        }
        printInfo("[org.smallbox.faraway.core.game.module.room.RoomModule] roof ok");
