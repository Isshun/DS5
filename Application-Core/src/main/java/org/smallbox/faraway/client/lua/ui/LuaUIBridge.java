...
public abstract class LuaUIBridge implements LuaExtendInterface {
    ...
    public View find(String id) { return DependencyManager.getInstance().getDependency(UIManager.class).findById(id); }
    public boolean  isVisible(String id) { return DependencyManager.getInstance().getDependency(UIManager.class).isVisible(id); }
