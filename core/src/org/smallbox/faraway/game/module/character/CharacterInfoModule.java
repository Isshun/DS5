...
public class CharacterInfoModule extends GameUIModule {
    ...
    private class CharacterInfoModuleWindow extends UIWindow {
        private final OnGaugeRefresh[] GAUGES = new OnGaugeRefresh[] {
        ...
        private ViewFactory         _viewFactory = ViewFactory.getInstance();
        ...
        protected void onCreate(UIWindow window, FrameLayout content) {
            _view = content;
            _view.findById("bt_monitoring").setOnClickListener(v -> switchView("frame_monitoring"));
            _view.findById("bt_personal_report").setOnClickListener(v -> switchView("frame_personal_report"));
            _view.findById("bt_priorities").setOnClickListener(v -> switchView("frame_priorities"));
            _view.findById("bt_inventory").setOnClickListener(v -> switchView("frame_inventory"));

            _cursor = ViewFactory.getInstance().createColorView(8, 16);
            _cursor.setBackgroundColor(Colors.LINK_INACTIVE);
            content.addView(_cursor);
            ...
            _view.findById("frame_monitoring").setVisible(true);
            _view.findById("frame_personal_report").setVisible(false);
            _view.findById("frame_priorities").setVisible(false);
            _view.findById("frame_inventory").setVisible(false);

            _lbName = (UILabel) _view.findById("lb_name");
            _lbJob = (UILabel) _view.findById("lb_current_job");

            FrameLayout frameBuffs = (FrameLayout)_view.findById("frame_buffs_entries");
            ...
            FrameLayout frameDiseases = (FrameLayout)_view.findById("frame_health_entries");
            ...
            _view.findById("frame_equipment_detail").setVisible(false);
...
//        }
            ...
            if (_character != null) {
                ...
                ((UILabel)_view.findById("lb_body_heat")).setString("Body heat: " + (int)(_character.getBodyHeat() * 10) / 10f);
            }
        ...
        protected String getContentLayout() {
            return "panels/info_character.yml";
        }

        private void switchView(String viewId) {
            _view.findById("frame_monitoring").setVisible("frame_monitoring".equals(viewId));
            _view.findById("frame_priorities").setVisible("frame_priorities".equals(viewId));
            _view.findById("frame_personal_report").setVisible("frame_personal_report".equals(viewId));
            _view.findById("frame_inventory").setVisible("frame_inventory".equals(viewId));
        }

        private void createTalents() {
            _priorityViews = new ArrayList<>();

            FrameLayout framePriorities = (FrameLayout) _view.findById("frame_priorities_entries");
...
//            framePriority.setBackgroundColor(new Color(0x88121c1e));
                ...
                lbProgress.setAlign(View.Align.CENTER);
...
//            framePriority.addView(btPriorityDown);
        ...
        private void createBasicInformation() {
            _lbOld = (UILabel) _view.findById("lb_info_old");

            _view.findById("lb_info_gender").setOnClickListener(view -> UserInterface.getInstance().getSelector().select(ToolTips.GENDER));

            _lbEnlisted = (UILabel) _view.findById("lb_info_enlisted");

            _lbBirthName = (UILabel) _view.findById("lb_info_birtname");
        }

        private void createInventoryInfo() {
            _lbInventory = (UILabel) _view.findById("lb_inventory");

            FrameLayout frameInventoryEntries = (FrameLayout)_view.findById("frame_inventory_entries");
        ...
        private void createNeedsInfo() {
            for (int i = 0; i < GAUGES.length; i++) {
                switch (i) {
                    case 0:
                        _values[i] = (UILabel)_view.findById("lb_status_food");
                        _shapes[i] = (UIImage)_view.findById("img_status_food");
                        break;
                    case 1:
                        _values[i] = (UILabel)_view.findById("lb_status_o2");
                        _shapes[i] = (UIImage)_view.findById("img_status_o2");
                        break;
                    case 2:
                        _values[i] = (UILabel)_view.findById("lb_status_happiness");
                        _shapes[i] = (UIImage)_view.findById("img_status_happiness");
                        break;
                    case 3:
                        _values[i] = (UILabel)_view.findById("lb_status_power");
                        _shapes[i] = (UIImage)_view.findById("img_status_power");
                        break;
                    case 4:
                        _values[i] = (UILabel)_view.findById("lb_status_relation");
                        if (_values[i] != null) {
                            _values[i].setOnFocusListener(null);
                            _values[i].setOnClickListener(view -> switchView("frame_personal_report"));
                        }
                        _shapes[i] = (UIImage)_view.findById("img_status_relation");
                        break;
                    case 5:
                        _values[i] = (UILabel)_view.findById("lb_status_security");
                        _shapes[i] = (UIImage)_view.findById("img_status_security");
                        break;
                    case 6:
                        _values[i] = (UILabel)_view.findById("lb_status_health");
                        if (_values[i] != null) {
                            _values[i].setOnFocusListener(null);
                            _values[i].setOnClickListener(view -> switchView("frame_health"));
                        }
                        _shapes[i] = (UIImage)_view.findById("img_status_health");
                        break;
                    case 7:
                        _values[i] = (UILabel)_view.findById("lb_status_joy");
                        _shapes[i] = (UIImage)_view.findById("img_status_joy");
                        break;
                }
        ...
        private void createRelationShip() {
            FrameLayout layoutFamily = (FrameLayout) _view.findById("frame_relationship");
...
//				_familyEntries[i].setStyle(TextView.UNDERLINED);
                    _familyEntries[i].setOnClickListener(view -> UserInterface.getInstance().getSelector().select(relation.getSecond()));
        ...
        private void refreshStatsBody() {
            _view.findById("frame_stats_body").setVisible(false);

            String body = _character.getType().label;
            ((UILabel) _view.findById("lb_ethnic_group")).setString(null);
            for (ItemInfo info: _character.getEquipments()) {
                if ("body".equals(info.equipment.location)) {
                    _view.findById("frame_stats_body").setVisible(true);
                    ...
                    ((UILabel) _view.findById("lb_stats_buff")).setString(buffValue);
                    ...
                    ((UILabel) _view.findById("lb_stats_absorb")).setString(absorbValue);
                    ...
                    ((UILabel) _view.findById("lb_stats_resist")).setString(resistsValue);

                    ((UILabel) _view.findById("lb_ethnic_group")).setString(body);
                }
            }
        }
        ...
        private void refreshStats() {
            ...
            ((UILabel) _view.findById("lb_total_buff")).setString(buffValue);
            ...
            ((UILabel) _view.findById("lb_total_absorb")).setString(absorbValue);
            ...
            ((UILabel) _view.findById("lb_total_resist")).setString(resistsValue);
        }
        ...
        private void createTimeTable() {
            FrameLayout frame = (FrameLayout)_view.findById("frame_timetable_entries");
            ...
            for (int h = 0; h < Game.getInstance().getPlanet().getInfo().dayDuration; h++) {
                ...
                lbRow.setAlign(View.Align.CENTER_VERTICAL);
...
//                    _selectedPriority.setPosition(0, i * 28);
        ...
        private void onCharacterSelect(CharacterModel character) {
            // Display need frame
            FrameLayout frameNeeds = (FrameLayout) _view.findById("frame_needs");
            ...
            FrameLayout frameEquipmentBody = (FrameLayout) _view.findById("frame_equipment_body");
        ...
        private void refreshStatsEquipments() {
            if (_character.getEquipmentViewIds() != null) {
                for (String[] equipmentViewId : _character.getEquipmentViewIds()) {
                    setEquipment((UILabel)_view.findById(equipmentViewId[0]), equipmentViewId[1]);
                }
...
//                        }
                ...
                ((UILabel) _view.findById("lb_equipment_total_resist")).setString("Resists: " + String.join(", ", resists));
                ...
                ((UILabel) _view.findById("lb_equipment_total_absorb")).setString("Absorbs: " + String.join(", ", absorb));
                ...
                ((UILabel) _view.findById("lb_equipment_total_buff")).setString("Buffs: " + String.join(", ", buff));
            }
        }

        private void refreshDebug() {
            if (_character != null) {
                FrameLayout frame = (FrameLayout)_view.findById("frame_debug");
                frame.removeAllViews();
        ...
        private void refreshHealth() {
            if (_character.getStats().deathMessage != null) {
                ((UILabel) _view.findById("lb_health")).setString(_character.getStats().deathMessage);
            }
        ...
        private void refreshInfo() {
            ...
            if (gender != null && !gender.equals(_lastGender)) {
                _lastGender = gender;
                startAnim((UILabel)_view.findById("lb_info_gender"), StringUtils.getDashedString("Gender:", CharacterInfoModel.Gender.MALE.equals(gender) ? "male" : "female", NB_COLUMNS));
                return;
            }
...
//                _character.getInventorySpace() - _character.getInventoryLeftSpace() + "/" + _character.getInventorySpace(), 29));
            ...
            ((UILabel) _view.findById("lb_inventory_entry")).setString(_character.getInventory() != null ?
...
//        }
        ...
        private void selectEquipment(View view, ItemInfo info) {
            for (String[] equipmentViewId: _character.getEquipmentViewIds()) {
                _view.findById(equipmentViewId[0]).setBackgroundColor(COLOR_BUTTON_INACTIVE);
            }

            view.setBackgroundColor(COLOR_BUTTON_ACTIVE);

            _view.findById("frame_equipment_detail").setPosition(view.getPosX(), view.getPosY() + 280);
            _view.findById("frame_equipment_detail").setVisible(false);

            if (info != null) {
                _view.findById("frame_equipment_detail").setVisible(true);
//
...
//                            resists.add(entry.getKey() + ": " + (entry.getValue() > 0 ? "+" + entry.getValue() : entry.getValue()));
//                        }
//                        ((UILabel)_view.findById("lb_equipment_resist")).setString("[R]: " + String.join(", ", resists));
//                    }
//
//                    // Check debuff
//                    if (effect.debuff != null) {
...
//                        }
//                        ((UILabel)_view.findById("lb_equipment_absorb")).setString("[A]: " + String.join(", ", absorbs));
//                    }
//
//                    // Check buff
//                    if (effect.buff != null) {
...
//                        }
//                        ((UILabel)_view.findById("lb_equipment_resist")).setString("[B]: " + String.join(", ", buffs));
//                    }
//                }
//            }
            }
...
//                if (_view.findById("frame_priorities").isVisible()) {
//                    // Pressed
...
//    }
        ...
        protected void addDebugView(String text, int x, int y, OnClickListener clickListener) {
            ...
            lbCommand.setOnClickListener(clickListener);
            lbCommand.resetSize();
            _view.addView(lbCommand);
        }

        private void refreshNeed(UILabel lbNeed, UIImage imgNeed, String label, CharacterModel character, double value, CharacterTypeInfo.NeedInfo needInfo) {
    ...
    private CharacterInfoModuleWindow _window;

    @Override
    protected void onLoaded() {
        _window = new CharacterInfoModuleWindow();
        addWindow(_window);
    }
