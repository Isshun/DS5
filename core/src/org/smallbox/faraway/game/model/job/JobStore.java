...
public class JobStore extends BaseJobModel implements GameObserver {
    ...
    private JobStore(ParcelModel jobParcel) {
        super(null, jobParcel, new IconDrawable("data/res/ic_haul.png", 0, 0, 32, 32), null);
    }

    public static JobStore create(ConsumableModel consumable, StorageAreaModel storage) {
        if (consumable == null) {
            Log.error("onCreate JobHaul: consumable cannot be null");
            return null;
        }

        JobStore job = new JobStore(consumable.getParcel());
