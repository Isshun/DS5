...
public class WorldRenderer implements IRenderer {
	...
	public WorldRenderer(SpriteManager spriteManager) {
		...
		_layerStructure = new RenderLayer(Constant.WORLD_WIDTH * Constant.TILE_WIDTH, Constant.WORLD_HEIGHT * Constant.TILE_HEIGHT);
		_layerItem = new RenderLayer(Constant.WORLD_WIDTH * Constant.TILE_WIDTH, Constant.WORLD_HEIGHT * Constant.TILE_HEIGHT);
		...
		for (int i = toX-1; i >= fromX; i--) {
			for (int j = toY-1; j >= fromY; j--) {
				if (i >= 0 && j >= 0 && i < Constant.WORLD_WIDTH && j < Constant.WORLD_HEIGHT) {
					...
					if (ressource != null) {
						SpriteModel sprite = _spriteManager.getRessource(ressource);
						if (sprite != null) {
...
//		}
	...
	private void refreshSelected(Renderer renderer, RenderEffect effect, int frame, ItemBase item) {
		int x = item.getX();
		int y = item.getY();
		int offset = 0;
		switch (frame % 5) {
	...
	public void onDrawSelected(Renderer renderer, RenderEffect effect, double animProgress) {
