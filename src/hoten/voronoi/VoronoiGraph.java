...
public abstract class VoronoiGraph {
	...
	public void paint(int[][] map) {
        for (Center c : centers) {
        	...
        	int x = (int)(c.loc.x - Constant.WORLD_WIDTH / 2);
        	int y = (int)(c.loc.y - Constant.WORLD_HEIGHT / 2);
        	...
        	// clean
        	if (x+1 >= 0 && y >= 0 && x+1 < Constant.WORLD_WIDTH && y < Constant.WORLD_HEIGHT && map[x+1][y] > e) { map[x+1][y] = e; }
        	if (x-1 >= 0 && y >= 0 && x-1 < Constant.WORLD_WIDTH && y < Constant.WORLD_HEIGHT && map[x-1][y] > e) { map[x-1][y] = e; }
        	if (x >= 0 && y+1 >= 0 && x < Constant.WORLD_WIDTH && y+1 < Constant.WORLD_HEIGHT && map[x][y+1] > e) { map[x][y+1] = e; }
        	if (x >= 0 && y-1 >= 0 && x < Constant.WORLD_WIDTH && y-1 < Constant.WORLD_HEIGHT && map[x][y-1] > e) { map[x][y-1] = e; }
        }
	}
