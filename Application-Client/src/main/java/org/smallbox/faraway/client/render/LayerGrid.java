...
public class LayerGrid {
    ...
    public void refresh() {
        for (int column = 0; column < _columns; column++) {
            for (int row = 0; row < _rows; row++) {
                if (_layers[column][row].isVisible(ApplicationClient.dependencyInjector.getObject(LayerManager.class).getViewport()) && _layers[column][row].needRefresh()) {
    ...
    public void draw(GDXRenderer renderer) {
        Viewport viewport = ApplicationClient.dependencyInjector.getObject(LayerManager.class).getViewport();
