...
public class LuaBuffExtend extends LuaExtend {
    ...
    public void extend(ModuleBase module, Globals globals, LuaValue value, File dataDirectory) throws DataExtendException {
        ...
        buff.setListener(new BuffInfo.BuffListener() {
            @Override
            public void onStart(BuffModel data) {
                ...
                if (!onStart.isnil()) {
                    onStart.call(data.luaCharacter, data.luaData);
                }
            }

            @Override
            public void onCheck(BuffModel data, int tick) {
                if (!onCheck.isnil()) {
                    LuaValue ret = onCheck.call(data.luaCharacter, data.luaData);

                    // Buff has not expire
                    if (data.active && data.info.getDuration() > 0 && data.info.getDuration() < (tick - data.startTick)) {
                        return;
                    }
            ...
            @Override
            public void onUpdate(BuffModel data, int tick) {
                if (data.active) {
...
//                                                    data.character.addDisease(new DiseaseModel(disease, data.luaCharacter, data.character, effectData));
//                                                }
...
//                                                }
                                                ...
                                                throw new GameException(LuaBuffExtend.class, "Unable to find disease: " + luaEffect.get("disease").toString());
                                            }
                                        }
...
//                                            data.character.getStats().isAlive = false;
            ...
            @Override
            public void onUpdateHourly(BuffModel data, int update) {
