package org.smallbox.faraway.common.lua.data;
...
public abstract class LuaExtend {
    ...
    protected <T> void readAsync(LuaValue value, String key, Class<T> cls, ReadCallback<T> callback) {
        LuaValue v = value.get(key);
        if (!v.isnil()) {
            getAsync(v.tojstring(), cls, callback::onReadCallback);
        }
    }

    protected abstract <T> void getAsync(String itemName, Class<T> cls, DataAsyncListener<T> dataAsyncListener);
