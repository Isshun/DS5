...
public class AStarPathFinder implements PathFinder, PathFindingContext {
    ...
    public AStarPathFinder(TileBasedMap map, int maxSearchDistance,
                           boolean allowDiagMovement, Node[][] nodes, Step[][] steps, AStarHeuristic heuristic) {
    ...
    public Path findPath(Mover mover, int sx, int sy, int tx, int ty) {
        ...
        while (target != nodes[sx][sy]) {
            path.prependStep(steps[target.x][target.y]);
            target = target.parent;
        }
        path.prependStep(steps[sx][sy]);
